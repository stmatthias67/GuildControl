...
client.on("interactionCreate", async (interaction) => {
  ...
  if (interaction.isChatInputCommand()) {
    ...
    try {
      ...
      } catch (e) { console.error("❌ Fehler beim Error Reply:", e); }
    }
    return;
  }
  ...
  if (
    ...
    if (
      ...
      try {
        ...
        try {
          if (!interaction.replied && !interaction.deferred) {
            await interaction.reply({ content: "❌ Fehler im Ticket-System.", ephemeral: true });
          }
    ...
    if (isSetupInteraction) {
      try {
        ...
        try {
          if (!interaction.replied && !interaction.deferred) {
            await interaction.reply({ content: "❌ Fehler im Setup-System.", ephemeral: true });
          }
        } catch (e) { console.error(e); }
      }
      return;
