...
async function handleRankSetupInteraction(interaction) {
  console.log("[RANK START]", interaction.customId);

  const id = interaction.customId;
  const guildId = interaction.guild.id;

  const modalIds = [
    "ranksetup-grundeinstellungen",
    "ranksetup-levelup-message",
    "ranksetup-leaderboard-pagesize",
  ];

  const opensModal =
    modalIds.includes(id) || id.startsWith("ranksetup-rangrollen-level:");
  ...
  if (id === 'ranksetup-toggle') {
    await interaction.deferUpdate().catch(() => null);  // FIX: deferUpdate statt nichts
    const rankConfig = await getOrCreateRankConfig(guildId);
...
if (id === 'ranksetup-back-main') {
  ...
  if (id === "ranksetup-modal-grundeinstellungen") {
    ...
    const cd = parseInt(
      interaction.fields.getTextInputValue("xp_cooldown"),
      10,
    );
    const mult = parseFloat(
      interaction.fields.getTextInputValue("xp_multiplier"),
    );

    // Validierung
    if (isNaN(xpMin) || isNaN(xpMax) || isNaN(cd) || isNaN(mult)) {
      return interaction.reply({
        content: "❌ Ungültige Eingabe. Bitte nur Zahlen eingeben.",
        ephemeral: true,
      });
    }
    if (xpMin < 1 || xpMax < xpMin) {
      return interaction.reply({
        content: "❌ Min XP muss kleiner als Max XP sein.",
        ephemeral: true,
      });
    }
    if (cd < 0) {
      return interaction.reply({
        content: "❌ Cooldown kann nicht negativ sein.",
        ephemeral: true,
      });
    }
    if (mult <= 0) {
      return interaction.reply({
        content: "❌ Multiplikator muss größer als 0 sein.",
        ephemeral: true,
      });
    }
  ...
  if (id.startsWith("ranksetup-rangrollen-create:")) {
    ...
    try {
      ...
      const existingRole = interaction.guild.roles.cache.find(
        (r) => r.name === `${slot.emoji} ${slot.label}`,
      );
  ...
  if (id.startsWith("ranksetup-modal-ranglevel:")) {
    const slotKey = id.split(":")[1];
    const levelRequired = parseInt(
      interaction.fields.getTextInputValue("level_required"),
      10,
    );

    if (isNaN(levelRequired) || levelRequired < 1) {
      return interaction.reply({
        content: "❌ Bitte eine gültige Zahl (≥ 1) eingeben.",
        ephemeral: true,
      });
    }
  ...
  if (id === "ranksetup-modal-leaderboard-pagesize") {
    const size = parseInt(
      interaction.fields.getTextInputValue("page_size"),
      10,
    );

    if (isNaN(size) || size < 1 || size > 25) {
      return interaction.reply({
        content: "❌ Bitte eine Zahl zwischen 1 und 25 eingeben.",
        ephemeral: true,
      });
    }
