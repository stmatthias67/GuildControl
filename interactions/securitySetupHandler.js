...
async function showMainSetup(interaction) {
  // Zurück zum /setup Hauptmenü (repliziert setup.js Antwort)
  ...
  const buttons = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId("setup-create-all").setLabel("🚀 Auto Setup").setStyle(ButtonStyle.Success),
    new ButtonBuilder().setCustomId("setup-finish").setLabel("✅ Setup Abschließen").setStyle(ButtonStyle.Primary),
...
async function handleMenuSelect(interaction, cfg) {
  const value = interaction.values[0];
  const views = {
    general:      () => interaction.editReply({ embeds: [buildGeneralEmbed(cfg)],      components: buildGeneralComponents() }),
    spam:         () => interaction.editReply({ embeds: [buildSpamEmbed(cfg)],          components: buildSpamComponents(cfg) }),
    links:        () => interaction.editReply({ embeds: [buildLinksEmbed(cfg)],         components: buildLinksComponents(cfg) }),
    mentions:     () => interaction.editReply({ embeds: [buildMentionsEmbed(cfg)],      components: buildMentionsComponents(cfg) }),
    caps:         () => interaction.editReply({ embeds: [buildCapsEmbed(cfg)],          components: buildCapsComponents(cfg) }),
    verification: () => interaction.editReply({ embeds: [buildVerificationEmbed(cfg)], components: buildVerificationComponents(cfg) }),
    antiraid:     () => interaction.editReply({ embeds: [buildAntiRaidEmbed(cfg)],      components: buildAntiRaidComponents(cfg) }),
    roles:        () => interaction.editReply({ embeds: [buildRolesEmbed(cfg)],         components: buildRolesComponents() }),
    lockdown:     () => interaction.editReply({ embeds: [buildLockdownEmbed(cfg)],      components: buildLockdownComponents(cfg) }),
  };
  if (views[value]) await views[value]().catch(() => null);
}
